...
class EzLinksPlugin(BasePlugin):
    config_scheme = (
        ("wikilinks", Type(bool, default=True)),
        ("warn_ambiguities", Type(bool, default=False)),
        ("reference_links", Type(bool, default=False)),
    )
    ...
    def on_files(self, files: List[File], config):
        self.file_mapper = FileMapper(
            options=EzLinksOptions(**self.config), root=config["docs_dir"], files=files, logger=LOGGER
        )
